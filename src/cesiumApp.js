...
async function updateWeatherEffects(viewer) {
  ...
  if (weatherParticleSystem) viewer.scene.primitives.remove(weatherParticleSystem);
...
async function updateBusEntities(viewer) {
  ...
  busData.forEach(bus => {
    ...
    if (existingBus) {
      ...
      const newBus = viewer.entities.add({ id: bus.id, position, orientation, model: { uri: '/models/bus.glb', minimumPixelSize: 64 }, label: { text: `[${bus.busNumber}]`, font: '12pt sans-serif', pixelOffset: new Cesium.Cartesian2(0, -50), style: Cesium.LabelStyle.FILL_AND_OUTLINE, outlineWidth: 2, } });
...
function showRestaurantPins(viewer, shops) {
  ...
  const pinIconUrl = pinBuilder.fromColor(Cesium.Color.fromCssColorString('#FF5733'), 48).toDataURL();
  shops.forEach(shop => {
    const position = Cesium.Cartesian3.fromDegrees(shop.lng, shop.lat, 30);
    const pinEntity = viewer.entities.add({
      position,
      billboard: { image: pinIconUrl, verticalOrigin: Cesium.VerticalOrigin.BOTTOM },
      label: { text: shop.name, font: '12pt sans-serif', style: Cesium.LabelStyle.FILL_AND_OUTLINE, outlineWidth: 2, verticalOrigin: Cesium.VerticalOrigin.BOTTOM, pixelOffset: new Cesium.Cartesian2(0, -55) },
...
function createDescriptionHtml(pickedFeature, shops) {
  ...
  } catch (e) { console.error("地物情報の処理中にエラー:", e); }
...
export async function startCesium(containerId) {
  ...
  } catch (error) { console.error("タイルセットの読み込みに失敗:", error); }
